...
function readBorder() {
  ...
  if (maxCnt != undefined) {
    ...
    cv.rectangle(src, new cv.Point(vertices[0].x, vertices[0].y),
  new cv.Point(vertices[3].x, vertices[3].y), (255,0,0,1), 3);
  }
  ...
  cv.imshow('canvas3', src); // SHOW CAMERA IMAGE
...
function scanCard() {
  ...
  cv.adaptiveThreshold(srcP, srcP, 255, cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY, 5, 2);
